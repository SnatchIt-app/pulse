...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  if (!yacht) return buildMetadata({ path: `/yachts/${slug}` });
...
export default async function YachtPage({ params }: { params: Promise<Params> }) {
  const { slug } = await params;
  const yacht = yachts.find((y) => y.slug === slug);
  if (!yacht) notFound();

  const title = getDisplayTitle(yacht);
  const heroImage = yacht.images?.[0] ?? null;
  const requestHref = `/request?yacht=${slug}&title=${encodeURIComponent(title)}`;

  const jsonLd = buildBreadcrumbJsonLd([
    { name: "Yachts", path: "/yachts" },
    { name: title, path: `/yachts/${slug}` },
  ]);

  // Build spec rows — only fields with verified data
  const specs: { label: string; value: string }[] = [
    { label: "Builder", value: yacht.make },
    ...(yacht.model ? [{ label: "Model", value: yacht.model }] : []),
    ...(yacht.length_ft ? [{ label: "Length", value: `${yacht.length_ft} ft` }] : []),
        ...
        <Container>
          <MotionFade>
            ...
            <p className="text-ink/45 text-[10px] uppercase tracking-[0.24em]">
              <Link href="/yachts" className="transition-colors hover:text-ink">
                Yachts
              </Link>
